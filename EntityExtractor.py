...
from lib.utils import ENTITIES_OF_INTEREST, RELATIONS, SEED_PROMPTS, SUBJ_OBJ_REQUIRED_ENTITIES
...
class spaCyExtractor:
    ...
    def get_relations(self, text):
        """
        Exposed function to take in text and return candidate relations with spaCy.
        """
        doc = self.process(text)
        target_candidate_pairs = self.extract_candidate_pairs(doc)

        # relations = self.extract_entities(target_candidate_pairs)
